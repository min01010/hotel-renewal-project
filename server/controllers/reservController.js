...
exports.createUserReservation = async (req, res) => {
  ...
  const { user_unique_id, room_id, rooms, check_in_date, check_out_date, total_price, termsAgreement } = reservations;

  try {
    // 예약 생성
    const reservationIdsData = await reservService.saveUserReservation(user_unique_id, room_id, rooms, check_in_date, check_out_date, total_price);

    // 약관 동의 정보 저장
    if (termsAgreement) {
      await reservService.insertTermsAgreement(reservationIdsData, termsAgreement);
    }
